...
class SettingsNotificationsController extends State<SettingsNotifications> {
  ...
  void testPushNotification() async {
    ...
    if (matrix.backgroundPush != null) {
      await PushTest.showTestNotification(
        matrix.backgroundPush!.flutterLocalNotificationsPlugin,
      );
      if (mounted) {
  ...
  void debugPushers() async {
    await PusherDebug.debugPushers(Matrix.of(context).client);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Check logs for pusher details')),
      );
    }
  }

  void recreatePusher() async {
    final matrix = Matrix.of(context);
    if (matrix.backgroundPush != null) {
      setState(() {
        isLoading = true;
      ...
      try {
        await matrix.backgroundPush!.setupUp(); // Перенастраиваем UnifiedPush
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('UnifiedPush setup started! Wait for new endpoint.')),
          );
        }
      } catch (e) {
        if (mounted) {
