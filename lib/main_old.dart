...
void onNotificationResponse(NotificationResponse response) {
  ...
  Logs().d('[Main] actionId: ${response.actionId}, payload: ${response.payload}, input: ${response.input}');
  ...
  try {
    if (response.actionId != null) {
      if (response.actionId!.startsWith('reply_')) {
        ...
        Logs().d('[Main] Reply: "$message" to room: $roomId');
        ...
        Logs().d('[Main] Mark read room: $roomId');
      ...
      if (roomId != null) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          try {
            ...
            Logs().d('[Main] Failed to navigate to room: $e');
          }
        });
      }
    }
  } catch (e, s) {
    Logs().d('[Main] Error handling notification response: $e');
    Logs().d('[Main] Stack trace: $s');
  }
}
...
void _handleNotificationAction(String action, String roomId, String? input) {
  ...
  Logs().d('[Main] Queued notification action: $action for room $roomId');
}
...
void _processPendingNotificationActions(Client? client) {
  ...
  Logs().d('[Main] Processing ${_pendingNotificationActions.length} pending notification actions');
  ...
  for (final action in validActions) {
    ...
    Logs().d('[Main] Processing action: $actionType for room $roomId');
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      try {
        ...
        Logs().d('[Main] Failed to process pending action: $e');
      }
    });
  }
  ...
  Logs().d('[Main] Processed and cleared pending notification actions');
}
