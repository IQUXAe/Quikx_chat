    ...
    if (kIsWeb) {
      ...
      notification.action.then((actionStr) {
        try {
          ...
          // Обработка действия по умолчанию (клик по уведомлению)
          if (action == null && (actionStr == "default" || actionStr.isEmpty)) {
            ...
            switch (action) {
              case DesktopNotificationActions.seen:
                event.room.setReadMarker(
                  ...
                  Logs().w('Failed to mark as read', error);
                });
                break;
          ...
          Logs().e('Error handling notification action', e);
        }
      }).catchError((error) {
        Logs().w('Error in notification action handler', error);
      });
