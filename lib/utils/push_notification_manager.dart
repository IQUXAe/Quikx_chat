...
class PushNotificationManager {
  ...
  static PushNotificationManager get instance => _instance ??= PushNotificationManager._();
  ...
  Future<void> _initializeAndroid() async {
    ...
    Logs().i('[PushNotificationManager] Android setup ready');
  ...
  void _onNotificationResponse(NotificationResponse response) {
    try {
      Logs().i('[PushNotificationManager] Notification response: ${response.actionId}, payload: ${response.payload}');
      ...
      switch (response.actionId) {
        ...
        case 'test_ok':
        case 'test_dismiss':
          Logs().i('[PushNotificationManager] Test notification action: ${response.actionId}');
          break;
      }
    } catch (e) {
      Logs().e('[PushNotificationManager] Error handling notification response', e);
    }
  }
  ...
  void _markRoomAsRead(String roomId) {
    try {
      ...
      Logs().i('[PushNotificationManager] Marking room as read: $roomId');
    } catch (e) {
      Logs().e('[PushNotificationManager] Failed to mark room as read', e);
    }
  }
  ...
  Future<bool> setupAutomatically(BuildContext context, MatrixState matrix) async {
    ...
    try {
      Logs().i('[PushNotificationManager] Starting automatic setup');
      ...
      if (!hasPermissions) {
        ...
        Logs().w('[PushNotificationManager] Permissions denied');
        return false;
      }
      
      Logs().i('[PushNotificationManager] Permissions granted');
      ...
      if (Platform.isAndroid) {
        try {
          ...
          Navigator.of(context).pop();
          Logs().e('[PushNotificationManager] Setup error', e, s);
      ...
      Navigator.of(context).pop();
      Logs().e('[PushNotificationManager] Error during automatic setup', e, s);
      _showErrorDialog(context, '${l10n.pushNotificationSetupFailed}: $e');
      return false;
    }
    return false;
  ...
  Future<bool> _setupUnifiedPush(BuildContext context, MatrixState matrix) async {
    try {
      Logs().i('[PushNotificationManager] Setting up UnifiedPush');
      
      // Проверяем доступные дистрибьюторы
      final distributors = await UnifiedPushHelper.getAvailableDistributors();
      Logs().i('[PushNotificationManager] Available distributors: $distributors');
      
      if (distributors.isEmpty) {
        Logs().w('[PushNotificationManager] No UnifiedPush distributors found');
        return false;
      }

      // Проверяем текущий дистрибьютор
      var selectedDistributor = await UnifiedPushHelper.getCurrentDistributor();
      Logs().i('[PushNotificationManager] Current distributor: $selectedDistributor');
      ...
      if (selectedDistributor == null || selectedDistributor.isEmpty || !distributors.contains(selectedDistributor)) {
        ...
        Logs().i('[PushNotificationManager] Selected distributor: $selectedDistributor');
      }
      ...
      if (success) {
        Logs().i('[PushNotificationManager] UnifiedPush setup initiated successfully');
        // Ждем немного, чтобы регистрация завершилась
        await Future.delayed(const Duration(seconds: 2));
        return true;
      } else {
        Logs().e('[PushNotificationManager] Failed to setup UnifiedPush');
        return false;
      }
    } catch (e, s) {
      Logs().e('[PushNotificationManager] Error setting up UnifiedPush', e, s);
      return false;
    }
  }
